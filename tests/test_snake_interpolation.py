...
def test_normal_movement_interpolates_previous_to_current_cell():
    ...
    assert positions == [(5.5 * GRID_SIZE, 5 * GRID_SIZE),
                         (4.5 * GRID_SIZE, 5 * GRID_SIZE)]
    assert positions[0][0] - positions[1][0] == GRID_SIZE
...
def test_direction_change_uses_only_confirmed_corner_positions():
    ...
    game.previous_snake = [(5, 5), (4, 5)]
    game.snake = [(5, 4), (5, 5)]
    assert game.get_snake_render_positions(now=10.1) == [
...
def test_two_players_have_independent_interpolation_history():
    ...
    assert game.get_snake_render_positions(1, now=10.1) == [(1.5 * GRID_SIZE, GRID_SIZE)]
    assert game.get_snake_render_positions(2, now=10.1) == [(8 * GRID_SIZE, 7.5 * GRID_SIZE)]
...
def test_start_and_restart_reset_visual_history():
    ...
    game.start_game(1)
    assert game.previous_snake == game.snake
    assert game.get_snake_render_positions(now=game.last_update_time) == [
...
def test_render_interpolation_scales_with_view_grid_size(monkeypatch):
    ...
    for grid_size in (20, 30, 40):  # VIEW_SCALE 1.0, 1.5 und 2.0
        ...
        assert game.get_snake_render_positions(now=10.1) == [(1.5 * grid_size, grid_size)]
